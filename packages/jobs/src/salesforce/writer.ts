    ...
    const written = await tx
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          revenueBand: sql`excluded.revenue_band`,
    ...
    const written = await tx
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          industry: sql`excluded.industry`,
          state: sql`excluded.state`,
          // A record read without the flag must not reopen a closed deal.
          isClosed: sql`coalesce(excluded.is_closed, ${schema.opportunities}.is_closed)`,
          syncRunId: sql`excluded.sync_run_id`,
          updatedAt: sql`excluded.updated_at`,
        },
      })
