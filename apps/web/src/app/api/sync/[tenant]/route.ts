...
import { canSyncNow, isManualSyncThrottled } from '@zeeraa/core';
 ...
 * every five minutes — except for a Zeeraa admin, who is not throttled. A refusal is 429 with the sentence the button shows.
 *
...
export async function POST(
  ...
  const outcome = await runManualSync({
    ...
    throttle: isManualSyncThrottled(session.tenant.role),
  });
