 ...
 *
 * The months start at the first one with a decline recorded, so there are no
 * empty bars from before the lending package was capturing them. The reasons
 * come from `declineReasonSummary`, the same figures as the Data quality
 * card, with the declines that carry no reason as their own row rather than
 * left out.
 */
  ...
  return (
    <Card span={span}>
        ...
        <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-3">
          <div>
            <p className="flex items-center gap-1.5 text-[13px] font-medium text-text-2">
              Deals declined
              <InfoTip label="What this counts" align="start">
                Each deal counts once, even if it was declined, reopened and declined again.
              </InfoTip>
            </p>
            <p className="mt-1 text-[28px] font-semibold leading-[1.15] tabular text-text">
              {formatCount(total)}
            </p>
            <p className="mt-1 text-[13px] tabular text-text-2">{formatRangeLabel(range)}</p>
          </div>

          <div className="min-w-[240px] flex-1">
            {bars.length === 0 ? (
              <EmptyLine>No declines recorded yet.</EmptyLine>
            ) : (
              <MonthBars id="declines-per-month" bars={bars} noun={['decline', 'declines']} height={150} />
            )}
          </div>
        </div>
      </CardBody>
      ...
      <div className="border-t border-border px-5 py-3">
        ...
        {reasons.declined === 0 ? (
          <EmptyLine className="mt-1">No lender declines in this period.</EmptyLine>
        ) : (
          <>
            <p className="mt-0.5 text-[13px] tabular text-text-2">
              Reason given on {share(reasons.withReason, reasons.declined)} of{' '}
              {formatCount(reasons.declined)} lender declines
            </p>
